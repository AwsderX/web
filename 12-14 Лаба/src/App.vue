  ...
  created() {
    eventHandler.$on('chancheTheme',toSwitch => {
      if (toSwitch){
        ...
        document.documentElement.style.setProperty('--back-color-secondary','#d4a7ffaf')
        document.documentElement.style.setProperty('--border-color','#9221fb')
        document.documentElement.style.setProperty('--text-color-primary','#3b0072')
        document.documentElement.style.setProperty('--text-color-secondary','white')
        document.documentElement.style.setProperty('--back-color-other','#a54afa')
      }
      else{
    ...
    eventHandler.$on('createTask',decription => {
      if (decription === undefined)
        decription = 'Заголовок'
      var time = new Date()
        this.planning.push({id: this.card_count, title: `${decription} (${this.card_count})`,
                           time_start: time,time_end: time,description: 'пример описание',task_manager: 'пример владельца'});
        this.card_count++;
    }),
...
:root {
  ...
  --back-color-secondary: #d4a7ffaf;
  --back-color-other: #a54afa;
  --border-color: #9221fb;
  --text-color-primary: #3b0072;
